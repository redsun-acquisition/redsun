...
from .._config import PLUGIN_KEYS, ConfigurationError, load, validate_session
from .._manifest import ServiceEntry, discover
from ..services._transports import TRANSPORT_KEY
...
def load_configuration(
    ...
    ------
    ConfigurationError
        If the file does not validate, names no ``session``, or names a device,
        presenter or view without the plugin it comes from.
    """
    paths = [Path(config_path)]
    config = load(paths)
    validate_session(paths, config)
    # a session built from a file alone has no class of its own to be named
    # after, and two left unnamed could not be told apart
    unnamed = (
        []
        if config.get("session")
        else [
            (
                "session: a session built with from_config names itself; two left "
                "unnamed could not be told apart"
            )
        ]
    )
    unnamed += [
