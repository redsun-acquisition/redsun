...
class StorageView(QtView, Loggable):
    """View of where the session stores its data.

    Shows and edits the base directory of the session path provider bound to
    [`PATH_PROVIDER`][redsun.storage.PATH_PROVIDER], whose paths have the form::

        <base_dir>/<session>/<YYYY-MM-DD>/<plan>_<counter>

    The view never owns a provider: when nothing is bound to the key it shows a
    ...
    def inject_dependencies(self, container: VirtualContainer) -> None:
        """Resolve the session path provider and follow its base directory."""
    ...
    def update_base_dir(self, reading: dict[str, Reading[str]]) -> None:
        """Show the base directory from a reading of the provider's signal."""
