...
class DetectorSettingsWidget(BaseWidget):
    ...
    def build_params(self, detector_info: DetectorModelInfo) -> ParameterTree:
        ...
        """
        params = [
            {'name': 'Model Name', 'type': 'str', 'value': detector_info.model_name, 'readonly': True},
            {'name': 'Supported Engines', 'type': 'list', 'value': detector_info.supported_engines[0], 'readonly': True},
            {'name': 'Vendor', 'type': 'str', 'value': detector_info.vendor, 'readonly': True},
            {'name': 'Serial Number', 'type': 'str', 'value': detector_info.serial_number, 'readonly': True},
            {'name': 'Category', 'type': 'str', 'value': detector_info.category, 'readonly': True},
            {'name': 'Sensor Size', 'type': 'str', 'value': str(detector_info.sensor_size), 'readonly': True},
            {'name': 'Pixel Size', 'type': 'str', 'value': str(detector_info.pixel_size)},
            {'name': 'Exposure EGU', 'type': 'str', 'value': detector_info.exposure_egu},
        ]
        parameter = Parameter.create(name='Detector parameters', type='group', children=params)
        tree = ParameterTree()
