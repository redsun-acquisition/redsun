...
from redsun.storage import FrameSink, SessionPathProvider, StreamSpec
...
async def drive(
    ...
    """Put `count` frames through the sink."""
    for i in range(count):
        await sink.put(frame(i, shape=shape, dtype=dtype))
...
class TestRoundTrip:
    async def test_bounded_burst_writes_capacity_frames(
        ...
        storage.register(spec("cam", capacity=4))
        sink = storage.sink("cam")
        await drive(sink, 4)
        await storage.close()
    ...
    async def test_multi_key_burst_shares_one_store(
        ...
        storage.register(spec("cam", capacity=3))
        storage.register(spec("median", capacity=3))
        cam = storage.sink("cam")
        median = storage.sink("median")
        await drive(cam, 3)
    ...
    async def test_consecutive_bursts_write_distinct_stores(
        ...
        for _ in range(2):
            storage.register(spec("cam", capacity=2))
            sink = storage.sink("cam")
            await drive(sink, 2)
            await storage.close()
    ...
    async def test_unbounded_burst_accepts_arbitrary_frames(
        ...
        storage.register(spec("cam", capacity=None))
        sink = storage.sink("cam")
        await drive(sink, 7)
        await storage.close()
    ...
    async def test_oversized_frame_is_split_not_rejected(
        ...
        storage.register(spec("cam", shape=(8, 8), capacity=None))
        sink = storage.sink("cam")
        await sink.put(frame(0, shape=(16, 16)))
        await storage.close()
